...
static struct gpio_desc typec_sel_desc;
...
static int setup_typec(void)
{
	...
	if (ret) {
		...
		ret = dm_gpio_lookup_name("gpio@20_3", &per_12v_desc);
		if (ret) {
	...
	ret = dm_gpio_lookup_name("gpio@21_4", &typec_sel_desc);
	if (ret) {
		printf("%s lookup gpio@21_4 failed ret = %d\n", __func__, ret);
		return -ENODEV;
	}
	...
	dm_gpio_set_dir_flags(&typec_sel_desc, GPIOD_IS_OUT | GPIOD_IS_OUT_ACTIVE);
